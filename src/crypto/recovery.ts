...
export function createRecoveryFile(vaultId: string, recoverySecret: Uint8Array): RecoveryFileV1 {
  return { v: 1, type: "jevs-budget-recovery", vaultId, recoverySecret: bytesToBase64(recoverySecret), createdAt: new Date().toISOString() };
}
...
export function downloadRecoveryFile(file: RecoveryFileV1) {
  ...
  anchor.href = url;
  anchor.download = `jevs-budget-recovery-${file.vaultId}.json`;
  anchor.click();
...
export async function unwrapWithRecoveryFile(file: RecoveryFileV1, wrapped: ArrayBuffer, salt: Uint8Array, argon2: Argon2Params) {
  if (file.v !== 1 || (file.type !== "jevs-budget-recovery" && file.type !== "jayrr-budget-recovery")) {
