...
import { encryptJson } from "./envelope";
import { envelopeMetadata } from "./envelope";
...
export async function savePrivateRecords(
  ...
  input: { userId: string; vaultId: string; keyId: string; masterKey: CryptoKey; records: PrivateRecordInput[] },
) {
  const encrypted = await Promise.all(input.records.map(async (record) => {
    const envelope = await encryptJson(record.value, { userId: input.userId, recordId: record.recordId, kind: record.kind, keyId: input.keyId }, input.masterKey);
    const data = envelopeMetadata(envelope);
    return { ...data, expectedRevision: record.expectedRevision ?? null, deleted: record.deleted ?? false };
  }));
