...
export function VaultSecurityCard() {
  ...
  const finishLink = useCallback(
    async (record: VaultUnlockRecord, nextPasscode: string) => {
      const masterKey = getVaultMasterKey();
      if (!masterKey)
        throw new Error("Sign in again before linking your password.");
  ...
  async function setup() {
    if (!passcode)
      throw new Error("Enter the same password you use to sign in.");
    await createVaultWithPasscode(vaultClient, passcode);
    clearPendingPasscode();
    setPasscode("");
    setMessage(
      "Encryption is on. Store the downloaded recovery file somewhere safe.",
    );
  }
  ...
  async function unlockWithPassword() {
    ...
    if (!ok)
      throw new Error(
        "That password did not unlock the vault. Try your sign-in password, an older vault passcode, or the recovery file.",
      );
    clearPendingPasscode();
    setPasscode("");
  ...
  async function unlockWithLegacyPasscode() {
    ...
    setMessage(
      "Unlocked with the old vault passcode. Enter your current sign-in password to finish.",
    );
  }
  ...
  async function addPasskey() {
    const masterKey = getVaultMasterKey();
    if (!vault || !masterKey)
      throw new Error("Sign in again before adding a passkey.");
    const packageData = await registerPasskey(masterKey, vault.vaultId);
    await setPasskeyPackage(packageData);
    setMessage(
      "Passkey added. You can use it to unlock this vault on supported devices.",
    );
  }

  async function unlockWithPasskeyOption() {
    if (!vault?.passkeyCredentialId || !vault.passkeyWrappedMasterKey)
      throw new Error("No passkey is registered for this vault.");
    await commitVaultUnlock(
      vault.vaultId,
      await unlockWithPasskey(
        vault.passkeyCredentialId,
        vault.passkeyWrappedMasterKey,
        vault.vaultId,
      ),
    );
    const pending = peekPendingPasscode();
  ...
  async function handleRecovery(file: File) {
    ...
    if (parsed.vaultId !== vault.vaultId)
      throw new Error("That recovery file belongs to a different vault.");
    await commitVaultUnlock(
      vault.vaultId,
      await unwrapWithRecoveryFile(
        parsed,
        vault.recoveryWrappedMasterKey,
        new Uint8Array(vault.recoverySalt),
        vault.argon2,
      ),
    );
    const pending = peekPendingPasscode();
    ...
    setNeedsPasscodeLink(true);
    setMessage(
      "Recovery accepted. Enter your current sign-in password so the next login unlocks the vault.",
    );
  }
    ...
    try {
      ...
      setError(
        cause instanceof Error
          ? cause.message
          : "Could not update encryption settings.",
      );
    } finally {
      setBusy(false);
    }
    ...
    <section className="space-y-4 rounded-xl border border-[var(--border)] bg-[var(--surface)] p-6">
      ...
      {!vault ? (
        <>
          <p className="text-sm text-[var(--muted-foreground)]">
            Sign in normally and encryption is created automatically. If it did
            not, type that same password here.
          </p>
          ...
          <button
            ...
            className="inline-flex items-center gap-2 rounded-md bg-primary px-3 py-2 text-sm font-medium text-primary-foreground hover:bg-primary-hover disabled:opacity-60"
          >
            <Lock className="size-4" />
            {busy ? "Creating…" : "Set up encryption"}
          </button>
        </>
      ) : needsPasscodeLink ? (
        <>
          <p className="text-sm text-[var(--muted-foreground)]">
            Type your current sign-in password so the next login opens the
            ledger automatically.
          </p>
          <label className="block space-y-2 text-sm">
            <span className="text-[var(--muted-foreground)]">
              Current sign-in password
            </span>
            <input
              type="password"
              value={passcode}
          ...
          <button
            type="button"
            disabled={busy}
            onClick={() => void run(linkCurrentPassword)}
            className="inline-flex items-center gap-2 rounded-md bg-primary px-3 py-2 text-sm font-medium text-primary-foreground hover:bg-primary-hover disabled:opacity-60"
          >
            <Download className="size-4" />
            Save password as vault passcode
          </button>
        </>
      ) : unlocked ? (
        <div className="flex flex-wrap gap-2">
          {!vault.passkeyCredentialId ? (
            <button
              ...
              className="inline-flex items-center gap-2 rounded-md border border-[var(--border)] px-3 py-2 text-sm disabled:opacity-60"
            >
              <Fingerprint className="size-4" />
              Add passkey
            </button>
          ) : (
            <span className="inline-flex items-center gap-2 px-2 py-2 text-sm text-[var(--muted-foreground)]">
              <Fingerprint className="size-4" />
              Passkey enabled
            </span>
          )}
        ...
        <>
          <p className="text-sm text-[var(--muted-foreground)]">
            Use the same password you sign in with. A reset on this browser
            re-keys the vault by itself after you have unlocked here once.
          </p>
          <div className="flex flex-wrap gap-2">
            ...
            <button
              type="button"
              disabled={busy}
              onClick={() => void run(unlockWithPassword)}
              className="inline-flex items-center gap-2 rounded-md bg-primary px-3 py-2 text-sm font-medium text-primary-foreground hover:bg-primary-hover disabled:opacity-60"
            >
            ...
            {vault.passkeyCredentialId ? (
              <button
                type="button"
                disabled={busy}
                onClick={() => void run(unlockWithPasskeyOption)}
                className="inline-flex items-center gap-2 rounded-md border border-[var(--border)] px-3 py-2 text-sm disabled:opacity-60"
              >
                <Fingerprint className="size-4" />
                Use passkey
          ...
          <div className="flex flex-col items-start gap-2">
            {showLegacy ? (
              <div className="flex w-full flex-wrap gap-2">
                <label className="min-w-0 flex-1 space-y-2 text-sm">
                  ...
                  <input
                    ...
                    placeholder="Older vault passcode"
                    className="w-full rounded-md border border-[var(--border)] bg-[var(--background)] px-3 py-2 text-sm"
                  />
                </label>
                <button
                  type="button"
                  disabled={busy}
                  onClick={() => void run(unlockWithLegacyPasscode)}
                  className="inline-flex items-center gap-2 rounded-md border border-[var(--border)] px-3 py-2 text-sm disabled:opacity-60"
                >
                  Unlock with old passcode
                </button>
              </div>
            ) : (
              <button
              ...
              >
                I used a separate vault passcode before
              </button>
            )}
            <input
              ref={fileRef}
              ...
              onChange={(event) => {
                ...
                if (file) void run(() => handleRecovery(file));
                event.target.value = "";
              }}
            />
            <button
      ...
      {vault ? (
        <p className="text-xs text-[var(--muted-foreground)]">
          This browser can reopen the ledger after a password reset. A new
          browser still needs the recovery file. Losing password, recovery file,
          and this browser makes the data unrecoverable.
        </p>
      ) : null}
    </section>
  );
