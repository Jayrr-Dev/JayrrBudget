...
export default function PrivateVaultPage() {
  ...
  if (!vault) return <div className="mx-auto max-w-2xl space-y-4"><h1 className="text-3xl font-semibold">Private vault</h1><p className="text-[var(--muted-foreground)]">Sign in with your password to create the vault, or finish setup on the <Link className="text-[var(--accent)] underline" href="/profile">profile page</Link>.</p></div>;
  return <div className="mx-auto max-w-3xl space-y-8">
    ...
    {!unlocked ? <div className="rounded-xl border border-[var(--border)] bg-[var(--surface)] p-5"><p className="text-sm text-[var(--muted-foreground)]">Vault is locked. Sign in again, or unlock with your password on the <Link className="text-[var(--accent)] underline" href="/profile">profile page</Link>.</p></div> : <>
