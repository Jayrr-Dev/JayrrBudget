/**
 * Driving range → Entertainment / Recreational / Driving Range
 * Usage: npx tsx scripts/driving-range-to-recreational.ts [--run]
 */
...
async function main() {
  ...
  const where = `
    lower(coalesce(merchant_clean,'')) LIKE '%driving range%'
  ...
  const subId = await ensure("transaction_subcategories", "Driving Range", db);
