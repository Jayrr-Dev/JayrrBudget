...
function client(count: number, repeated = true, cached = false) {
  ...
  const rows = Array.from({ length: count }, (_, i) => ({
    transactionId: `t${i}`,
    description: repeated ? "ACME" : `ACME ${i}`,
    amount: 10,
    key: descriptionKey(repeated ? "ACME" : `ACME ${i}`, 10),
    updatedAt: 1,
  }));
  ...
  return {
    ...
    client: {
      query: async (fn: any, args: any) => {
        ...
        if (name.endsWith(":pendingPage"))
          return {
            page: rows.filter((r) => !saved.has(r.transactionId)),
            isDone: true,
            continueCursor: "",
          };
        ...
        if (name.endsWith("aiRules:get") || name.endsWith(":get"))
          return { rules: [] };
        throw new Error(name);
      },
      mutation: async (fn: any, args: any) => {
        if (getFunctionName(fn).endsWith(":publishOwnVocabulary")) return;
        if (getFunctionName(fn).endsWith(":ensurePaths")) return [];
        const count = args.groups.reduce(
          (n: number, g: any) => n + g.rows.length,
          0,
        );
        assert.ok(count <= 200);
        assert.ok(args.groups.length <= 40);
...
test("known descriptions skip AI entirely", async () => {
  ...
  const result = await categorizeStatement(fixture.client as any, 1);
  assert.equal(aiState.calls, 0);
  assert.equal(result.cached, 100);
  assert.equal(result.pending, 0);
});
test("81 unique descriptions use three bounded batches", async () => {
  const fixture = client(81, false);
  const result = await categorizeStatement(fixture.client as any, 1);
  assert.equal(aiState.calls, 3);
  assert.equal(result.ai, 81);
});
