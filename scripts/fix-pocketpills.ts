/**
 * PocketPills → Health / Medical / Prescriptions
 * Usage: npx tsx scripts/fix-pocketpills.ts [--run]
 */
...
async function main() {
  ...
  const where = `
    lower(coalesce(merchant_clean,'')) LIKE '%pocketpill%'
    OR lower(coalesce(merchant_name,'')) LIKE '%pocketpill%'
    OR lower(coalesce(description,'')) LIKE '%pocketpill%'
  `;
