...
export const pendingPage = query({
  args: { uploadId: v.number(), paginationOpts: paginationOptsValidator },
  handler: async (ctx, args) => {
    const user = await requireUser(ctx);
    ...
    const result = await ctx.db.query("transactions").withIndex("by_userId_statementUploadId", q => q.eq("userId", user._id).eq("statementUploadId", args.uploadId)).paginate(args.paginationOpts);
    return { ...result, page: result.page.filter(t => !isCategorized(t)).map(t => ({
      transactionId: t.transactionId, description: t.description, amount: t.amount,
      updatedAt: t.updatedAt, key: descriptionKey(t.description, t.amount),
    })) };
  },
});
...
export const apply = mutation({
  args: { groups: v.array(v.object({ key: v.string(), profile: profileValidator,
  ...
  handler: async (ctx, args) => {
    ...
    for (const group of args.groups) {
      ...
      for (const input of group.rows) {
        ...
        if (!row || row.updatedAt !== input.updatedAt || isCategorized(row)) continue;
