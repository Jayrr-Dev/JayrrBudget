...
const argon2Validator = v.object({ algorithm: v.literal("argon2id"), version: v.number(), timeCost: v.number(), memoryCost: v.number(), parallelism: v.number(), hashLength: v.number() });

export const get = query({
  args: {},
  returns: v.union(v.object({
    vaultId: v.string(), mode: modeValidator, status: statusValidator, currentKeyId: v.string(),
    passphraseWrappedMasterKey: v.bytes(), passphraseSalt: v.bytes(), recoveryWrappedMasterKey: v.bytes(), recoverySalt: v.bytes(),
    argon2: argon2Validator, passkeyWrappedMasterKey: v.union(v.bytes(), v.null()), passkeyCredentialId: v.union(v.string(), v.null()),
  ...
  handler: async (ctx) => {
    ...
    return {
      vaultId: vault.vaultId, mode: vault.mode, status: vault.status, currentKeyId: vault.currentKeyId,
      passphraseWrappedMasterKey: vault.passphraseWrappedMasterKey, passphraseSalt: vault.passphraseSalt,
      recoveryWrappedMasterKey: vault.recoveryWrappedMasterKey, recoverySalt: vault.recoverySalt,
      argon2: { algorithm: "argon2id" as const, version: vault.argon2Version, timeCost: vault.argon2TimeCost, memoryCost: vault.argon2MemoryCost, parallelism: vault.argon2Parallelism, hashLength: vault.argon2HashLength },
      passkeyWrappedMasterKey: vault.passkeyWrappedMasterKey ?? null, passkeyCredentialId: vault.passkeyCredentialId ?? null,
...
export const create = mutation({
  args: {
    vaultId: v.string(), mode: modeValidator, currentKeyId: v.string(),
  ...
  handler: async (ctx, args) => {
    const user = await requireUser(ctx);
    const existing = await ctx.db.query("vaults").withIndex("by_userId", (q) => q.eq("userId", user._id)).first();
    if (existing) return { vaultId: existing.vaultId, created: false };
    const now = Date.now();
    await ctx.db.insert("vaults", {
      userId: user._id, vaultId: args.vaultId, mode: args.mode, status: "active", currentKeyId: args.currentKeyId,
      passphraseWrappedMasterKey: args.passphraseWrappedMasterKey, passphraseSalt: args.passphraseSalt,
      recoveryWrappedMasterKey: args.recoveryWrappedMasterKey, recoverySalt: args.recoverySalt,
      argon2Version: args.argon2.version, argon2TimeCost: args.argon2.timeCost, argon2MemoryCost: args.argon2.memoryCost,
      argon2Parallelism: args.argon2.parallelism, argon2HashLength: args.argon2.hashLength, createdAt: now, updatedAt: now,
    });
...
export const saveRecords = mutation({
  args: {
    vaultId: v.string(),
    records: v.array(v.object({ recordId: v.string(), kind: v.string(), v: v.number(), alg: v.string(), keyId: v.string(), iv: v.bytes(), wrappedDek: v.bytes(), ciphertext: v.bytes(), expectedRevision: v.union(v.number(), v.null()), deleted: v.boolean() })),
  },
